...
def product_of_entries_which_sum_to(entries: Iterable[int], target: int) -> int:
    """
    --- Day 1: Report Repair ---
    ...
    if not isinstance(entries, set):
        entries = set(entries)
    candidates = [entry for entry in entries if target - entry in entries]
    assert len(candidates) == 2
    return candidates[0] * (target - candidates[0])


def product_of_three_entries_which_sum_to(entries: Iterable[int], target: int) -> int:
    """
    ...
    for entry in entries:
        ...
        try:
            two_product = product_of_entries_which_sum_to(entries_set, complement)
            return two_product * entry
